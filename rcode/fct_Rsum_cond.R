# 1.11 La fonction qui resume pour un SNPs une seule exposition
   ...
   fct_Rcond<-function(dat,outc,vexp,gm,gc,vvaraj,strate,seu=NULL){
                                            ...
                                            # le cas binaire où on precise le seuille de de la variable de dichotomisation
                                               nvar<-cathe_bi2(dat,outc,vexp,seu);nvar<-t(t(nvar));nch<-paste(vexp,".","dch",sep="");colnames(nvar)<-nch 
                                                                   ...
                                                                   fl0=formula(paste("Surv(2-",outc,",",outc,")~",paste(c(paste(c(nch,gm,gc),collapse="+"),paste(nch,":",gm,sep=""),paste(nch,":",gc,sep=""),paste(vvaraj,collapse="+"),paste("strata(",strate,")",sep="")),collapse="+"),sep=""))   
