# 1.11 La fonction qui resume pour un SNPs et 2 expositions
   ...
   fct_Rsum_2exp_ordre2_cond<-function(dat,outc,vexp,vexp2,gm,gc,vvaraj,strate,seu=NULL,seu2=NULL){
                                            ...
                                            # le cas binaire où on precise le seuille de de la variable de dichotomisation
                                               nvar<-cbind(cathe_bi2(dat,outc,vexp,seu),cathe_bi2(dat,outc,vexp2,seu2));nch<-paste(vexp,".","dch",sep="");nch2<-paste(vexp2,".","dch",sep="");colnames(nvar)<-c(nch,nch2) 
                                                                   ...
                                                                   fl0=formula(paste("Surv(2-",outc,",",outc,")~",paste(c(paste(c(nch,nch2,gm,gc),collapse="+"),paste(nch,":",gm,sep=""),paste(nch,":",gc,sep=""),paste(nch2,":",gm,sep=""),paste(nch2,":",gc,sep=""),paste(vvaraj,collapse="+"),paste("strata(",strate,")",sep="")),collapse="+"),sep=""))   
                                                                   mod0<-coxph(fl0,data=dat1,ties="exact")
                                                                   cof<-coef(mod0)
                                                                   Ic1<-confint(mod0)
